...
class ChatGptEmotionDetectionServiceImpl @Inject()(ws: WSClient, config: Configuration, aiService: AiService)(implicit ec: ExecutionContext) extends EmotionDetectionService {
            ...
            Try {
              response.json.validate[ChatGptApiResponse] match {
                case JsSuccess(result, _) =>
                  ...
                  saveAiResponseAsync(request, response, result.id)
  ...
  private def saveAiResponseAsync(request: DetectEmotionRequest, response: WSResponse, id: String): Unit = {
    Future(aiService.insert(
      AiDbObj(None, Json.stringify(response.json), request.userId, None), id)
    ).onComplete {
      case scala.util.Success(_) => logger.info(s"Successfully saved AI response: $id")
      case scala.util.Failure(e) => logger.error(s"Failed to save AI response: $id", e)
    }
  }
}
